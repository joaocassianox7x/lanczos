...
def lanczos_rkky_norht(A,m=0): #LANCZOS OF THE BELLOW PAPER, WITHOUT RE-ORTHOGONALIZATION
    ...
    psi[:,1]=A.dot(psi[:,0])-alpha[0]*psi[:,0] #psi_{n+1}
    beta[0]=.0
    beta[1]=(np.linalg.norm(psi[:,1])**2)/modu(psi[:,0]) #beta_1
    #next steps
    for i in range(2,interactions):
        a=A.dot(psi[:,i-1])-(beta[i-1])*psi[:,i-2]-alpha[i-1]*psi[:,i-1]
        psi[:,i]=a
...
def hamiltonian_honney(matriz): #TRANSFORM THE HONNEYCOMB LATTICE INTO A HAMILTONIAN (FIRST HOPPING ONLY)
    ...
    #print(vec)
    hamil = sparse.csr_matrix((num_elem+2, num_elem+2), dtype=np.float64)
...
t0=t.time()
spi=spiral_honney(221,221) # GENERATE THE LATTICE
...
if SAVE_DATA:
    ...
    io.mmwrite('hamiltonian_utilized.dat',c,precision=14)
